...
\maketitle
\begin{abstract}
\pysr is an open-source library for practical \textit{symbolic regression}, a type of machine learning which aims to discover human-interpretable symbolic models.
...
\pysr's backend is an extremely optimized Julia library \pysrjl, which can be used directly from Julia. It is capable of fusing user-defined operators into SIMD kernels at runtime, performing automatic differentiation, and distributing populations of expressions to thousands of cores across a cluster.
...
\cite{dascoliDeepSymbolicRegression2022,kamiennyEndtoendSymbolicRegression2022} which pre-train a large transformer model on billions of synthetic examples, in order to auto-regressively generate expressions from data;
...
and several different mutations and crossovers for generating new individuals.
...
Thus the probability will roughly be $p, p(1-p), p(1-p)^2, ...$ for the first, second, third fittest, and so on. 
\label{step:select}
...
This algorithm is computationally efficient and allows for massive paralellization by splitting step~\ref{step:sampling} into groups of individuals which can evolve independently, with asynchronous ``migration'' between the groups.
...
Its inclusion in \pysr was motivated by the impressive Neural Architecture Search results of~\cite{realRrealRegularizedEvolutionImage2019,realAutoMLZeroEvolvingMachine2020a} --- though it is worth noting similar regularizations used earlier in SR, e.g.,~\cite{hornbyALPSAgelayeredPopulation2006} and~\cite{schmidtAgefitnessParetoOptimization2010} --- which found this simple modification could prevent early convergence without hurting performance.
This prevents the population from specializing too quickly and getting stuck in a local minima of the search space.
...
Pseudocode for the main loop of \pysr is given in \cref{alg:pysr}.
\Cref{alg:evolve} gives pseudocode for the main \textit{evolve-simplify-optimize loop}.
...
\bibliography{Zotero_Library.bib,software.bib}
